...
---
--- @param feat Feat
--- @param repo string|nil "owner/name", or nil for non-thing-bound feats (e.g. "azdo://status").
--- @param id string|integer PR/issue number, or "all" for "azdo://status".
...
--- Sets the `b:azdo` buffer-local dict. `bufstate` is merged with existing state, if any.
--- @param buf integer
--- @param bufstate BufState
function M.set_b_azdo(buf, bufstate)
  local b_azdo = vim.b[buf].azdo
  if not b_azdo then
    vim.b[buf].azdo = bufstate
  else
    vim.b[buf].azdo = vim.tbl_extend('force', b_azdo, bufstate)
  end
end
...
function M.init_buf(feat, focus, repo, id, bufstate)
  ...
  -- XXX: Stash the key so anything (e.g. `util.run_term_cmd()`) can rebuild the `azdo://…` URL,
  -- even for "global" (non-repo-specific) buffers like `azdo://status`.
  bufstate.bufkey = key
  M.set_b_azdo(buf, bufstate)
  M.set_buf_name(buf, feat, key)
  return buf, key
end

--- Gets buffer URI:
--- - `azdo://<org>/<project>/<repo>/<feat>/<id>` for per-repo per-feature buffers.
--- - `azdo://<feat>` for "global" buffers (`azdo://status`).
---
--- The key for a repo-bound buffer is "<org>/<project>/<repo>/<id>", so split off the trailing
--- id; the leading (slash-containing) remainder is the repo triplet.
---
--- @param feat Feat
--- @param key string "<repo>/<id>", or "all"/<feat> for global buffers.
local function get_buf_name(feat, key)
  local repo, id = key:match('^(.+)/([^/]+)$')
  if repo then
    return ('azdo://%s/%s/%s'):format(repo, feat, id)
  end
  return ('azdo://%s'):format(feat)
end

--- Sets the buffer name to "azdo://…/…" format.
