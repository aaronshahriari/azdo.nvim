...
local diag_ns = vim.api.nvim_create_namespace('azdo.comments')
...
function M.show_pr_comments(id, repo, diff_win, comments_list, viewed, n_files, n_threads, n_resolved, n_viewed_threads)
  ...
  vim.diagnostic.setqflist({ namespace = diag_ns, title = 'Azdo comments', open = false })
  ...
  vim.api.nvim_buf_call(buf, function()
    vim.cmd([[syntax match AzdoHeading /^▎ \zs.*$/ containedin=ALL]])
    vim.cmd([[syntax match AzdoWarning /(outdated)\|(outside)\|(truncated)/ containedin=ALL]])
  end)
...
---
--- @param gh_comments table[] flat list from `az.get_pr_data`.
...
--- Prepares the output of the PR diff for display.
---
--- - Replaces "viewed" files with a single `(viewed) <path>` line
--- - Counts files in the diff
---
--- @param difftext string Raw the PR diff output.
...
--- Renders `the PR diff` stdout for presentation in a "prdiff/…" buffer:
---
...
--- @param diff_stdout string  Raw the PR diff output.
--- @return string[] lines
...
local function comments_at_line(linenr)
  local prnum, repo = util.require_b_azdo({ 'id', 'repo' })
  if not prnum then
    return
  end
...
function M.delete_comment(linenr)
  with_comment(linenr, 'Delete which comment?', function(c, prnum, repo, buf)
    ...
    az.delete_comment(c.id, repo, function(resp)
      if not resp or resp.errors == nil then
        progress('success', nil, 'Comment deleted.')
        require('azdo.pr').show_pr_diff(prnum) -- Refresh.
      else
...
function M.update_comment(linenr)
  with_comment(linenr, 'Which comment?', function(c, prnum, repo, _)
    ...
    M.edit_comment('comment', prnum, content, infomsg, function(input)
      ...
      az.update_comment(c.id, input, repo, function(resp)
  ...
  with_comment(linenr, 'Which comment thread?', function(c, prnum, repo, buf)
    vim.ui.select({ 'Reply', 'Resolve' }, {
      ...
      if action == 'Reply' then
        M.edit_comment(
          ...
          function(input)
            ...
            az.reply_to_comment(prnum, input, c.id, repo, function(resp)
              if resp['errors'] == nil then
      ...
      elseif action == 'Resolve' then
        ...
        az.resolve_thread(c.thread_node_id, function(resp)
          if resp['errors'] == nil then
...
function M.prepare_to_comment(line1, line2)
  local buf = vim.api.nvim_get_current_buf()
  local prnum = assert(vim.b.azdo.id)
  local repo = assert(vim.b.azdo.repo)
...
function M.new_comment(line1, line2)
  ...
  az.get_pr_data(info.pr_id, info.repo, nil, function(pr)
    if not pr then
    ...
    vim.schedule(function()
      M.edit_comment('comment', info.pr_id, { '' }, infomsg, function(input)
        ...
        az.new_comment(pr, input, info.file, info.start_line, info.end_line, info.side, info.repo, function(resp)
          if resp['errors'] == nil then
            progress('success', nil, 'Comment sent.')
            require('azdo.pr').show_pr_diff(info.pr_id) -- Refresh.
          else
...
function M.edit_comment(feat, prnum, content, infomsg, on_confirm)
  local repo = assert((vim.b.azdo or {}).repo, 'edit_comment: not in a azdo:// buffer (no b:azdo.repo)')
